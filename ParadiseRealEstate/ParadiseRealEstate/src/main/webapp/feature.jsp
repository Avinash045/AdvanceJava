...
<html lang="en">
	<head>
		...
		<title>Paradise Real Estate - Featured Property</title>
	</head>
	...
	<%
		...
		ResultSet query = con.createStatement().executeQuery("SELECT * FROM property WHERE uid="+uid);
		while(query.next()){
	%>
						<tr>
	                    	<td>
	                        ...
	                        <td><a class="btn btn-info" href="submitpropertyupdate.jsp?id=<%=query.getString(1)%>">Update</a></td>
							<td><a class="btn btn-danger" href="submitpropertydelete.jsp?id=<%=query.getString(1)%>">Delete</a></td>
	                 	</tr>
	<%
		}
	%>
